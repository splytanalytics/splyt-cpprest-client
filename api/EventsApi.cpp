...
pplx::task<void> EventsApi::createEvent(utility::string_t customerId, std::shared_ptr<NewEventRequest> request)
{
    ...
    {
        throw ApiException(400, U("EventsApi->createEvent does not produce any supported media type"));
    }
    ...
    {
        throw ApiException(415, U("EventsApi->createEvent does not consume any supported media type"));
    }
    ...
    .then([=](web::http::http_response response)
    {
        ...
        if (response.status_code() >= 400)
        {
            throw ApiException(response.status_code()
                , U("error calling createEvent: ") + response.reason_phrase()
        ...
        if(response.headers().has(U("Content-Type")))
        {
            ...
            if( contentType.find(responseHttpContentType) == std::string::npos )
            {
                throw ApiException(500
                    , U("error calling createEvent: unexpected response type: ") + contentType
                    , std::make_shared<std::stringstream>(response.extract_utf8string(true).get()));
            }
        }
