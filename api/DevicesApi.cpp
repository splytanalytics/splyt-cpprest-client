...
pplx::task<void> DevicesApi::newDevice(utility::string_t customerId, std::shared_ptr<DataCollectorNewDeviceRequest> request, bool checked)
{
    ...
    {
        throw ApiException(400, U("DevicesApi->newDevice does not produce any supported media type"));
    }
    ...
    {
        throw ApiException(415, U("DevicesApi->newDevice does not consume any supported media type"));
    }
    ...
    .then([=](web::http::http_response response)
    {
        ...
        if (response.status_code() >= 400)
        {
            throw ApiException(response.status_code()
                , U("error calling newDevice: ") + response.reason_phrase()
        ...
        if(response.headers().has(U("Content-Type")))
        {
            ...
            if( contentType.find(responseHttpContentType) == std::string::npos )
            {
                throw ApiException(500
                    , U("error calling newDevice: unexpected response type: ") + contentType
                    , std::make_shared<std::stringstream>(response.extract_utf8string(true).get()));
            }
        }
...
pplx::task<void> DevicesApi::updateDeviceState(utility::string_t id, utility::string_t customerId, std::shared_ptr<DataCollectorUpdateDeviceStateRequest> request)
{
    ...
    {
        throw ApiException(400, U("DevicesApi->updateDeviceState does not produce any supported media type"));
    }
    ...
    {
        throw ApiException(415, U("DevicesApi->updateDeviceState does not consume any supported media type"));
    }
    ...
    .then([=](web::http::http_response response)
    {
        ...
        {
            throw ApiException(response.status_code()
                , U("error calling updateDeviceState: ") + response.reason_phrase()
                , std::make_shared<std::stringstream>(response.extract_utf8string(true).get()));
        }
        ...
        if(response.headers().has(U("Content-Type")))
        {
            ...
            {
                throw ApiException(500
                    , U("error calling updateDeviceState: unexpected response type: ") + contentType
                    , std::make_shared<std::stringstream>(response.extract_utf8string(true).get()));
            }
        }
