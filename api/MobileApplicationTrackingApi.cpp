...
#include "MobileApplicationTrackingApi.h"
#include "IHttpBody.h"
#include "JsonBody.h"
...
MobileApplicationTrackingApi::MobileApplicationTrackingApi( std::shared_ptr<ApiClient> apiClient )
...
pplx::task<void> MobileApplicationTrackingApi::handleTuneRequestUsingPOST(utility::string_t customerId, std::shared_ptr<DataCollectorTuneRequest> request)
{
    

    std::shared_ptr<ApiConfiguration> apiConfiguration( m_ApiClient->getConfiguration() );
    utility::string_t path = U("/v2/tune");
    ...
    {
        throw ApiException(400, U("MobileApplicationTrackingApi->handleTuneRequestUsingPOST does not produce any supported media type"));
    }
    ...
    {
        throw ApiException(415, U("MobileApplicationTrackingApi->handleTuneRequestUsingPOST does not consume any supported media type"));
    }
    ...
    .then([=](web::http::http_response response)
    {
        ...
        if (response.status_code() >= 400)
        {
            throw ApiException(response.status_code()
                , U("error calling handleTuneRequestUsingPOST: ") + response.reason_phrase()
        ...
        if(response.headers().has(U("Content-Type")))
        {
            ...
            if( contentType.find(responseHttpContentType) == std::string::npos )
            {
                throw ApiException(500
                    , U("error calling handleTuneRequestUsingPOST: unexpected response type: ") + contentType
                    , std::make_shared<std::stringstream>(response.extract_utf8string(true).get()));
            }
        }
