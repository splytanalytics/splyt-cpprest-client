...
pplx::task<void> MobileApplicationTrackingApi::submitTuneRequest(utility::string_t customerId, std::shared_ptr<DataCollectorTuneRequest> request)
{
    ...
    {
        throw ApiException(400, U("MobileApplicationTrackingApi->submitTuneRequest does not produce any supported media type"));
    }
    ...
    {
        throw ApiException(415, U("MobileApplicationTrackingApi->submitTuneRequest does not consume any supported media type"));
    }
    ...
    .then([=](web::http::http_response response)
    {
        ...
        if (response.status_code() >= 400)
        {
            throw ApiException(response.status_code()
                , U("error calling submitTuneRequest: ") + response.reason_phrase()
        ...
        if(response.headers().has(U("Content-Type")))
        {
            ...
            if( contentType.find(responseHttpContentType) == std::string::npos )
            {
                throw ApiException(500
                    , U("error calling submitTuneRequest: unexpected response type: ") + contentType
                    , std::make_shared<std::stringstream>(response.extract_utf8string(true).get()));
            }
        }
