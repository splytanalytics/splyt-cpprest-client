...
pplx::task<void> UsersApi::newUser(utility::string_t customerId, std::shared_ptr<DataCollectorNewUserRequest> request, bool checked)
{
    ...
    {
        throw ApiException(400, U("UsersApi->newUser does not produce any supported media type"));
    }
    ...
    {
        throw ApiException(415, U("UsersApi->newUser does not consume any supported media type"));
    }
    ...
    .then([=](web::http::http_response response)
    {
        ...
        if (response.status_code() >= 400)
        {
            throw ApiException(response.status_code()
                , U("error calling newUser: ") + response.reason_phrase()
        ...
        if(response.headers().has(U("Content-Type")))
        {
            ...
            if( contentType.find(responseHttpContentType) == std::string::npos )
            {
                throw ApiException(500
                    , U("error calling newUser: unexpected response type: ") + contentType
                    , std::make_shared<std::stringstream>(response.extract_utf8string(true).get()));
            }
        }
...
pplx::task<void> UsersApi::updateUserState(utility::string_t id, utility::string_t customerId, std::shared_ptr<DataCollectorUpdateUserStateRequest> request)
{
    ...
    {
        throw ApiException(400, U("UsersApi->updateUserState does not produce any supported media type"));
    }
    ...
    {
        throw ApiException(415, U("UsersApi->updateUserState does not consume any supported media type"));
    }
    ...
    .then([=](web::http::http_response response)
    {
        ...
        {
            throw ApiException(response.status_code()
                , U("error calling updateUserState: ") + response.reason_phrase()
                , std::make_shared<std::stringstream>(response.extract_utf8string(true).get()));
        }
        ...
        if(response.headers().has(U("Content-Type")))
        {
            ...
            {
                throw ApiException(500
                    , U("error calling updateUserState: unexpected response type: ") + contentType
                    , std::make_shared<std::stringstream>(response.extract_utf8string(true).get()));
            }
        }
