...
pplx::task<void> BatchApi::batchUsingPOST(std::shared_ptr<DataCollectorBatchRequest> batchRequest, utility::string_t customerId)
{
    
    // verify the required parameter 'batchRequest' is set
    if (batchRequest == nullptr)
    {
        throw ApiException(400, U("Missing required parameter 'batchRequest' when calling BatchApi->batchUsingPOST"));
    }
    ...
    {
        throw ApiException(400, U("BatchApi->batchUsingPOST does not produce any supported media type"));
    }
    ...
    {
        throw ApiException(415, U("BatchApi->batchUsingPOST does not consume any supported media type"));
    }
    ...
    .then([=](web::http::http_response response)
    {
        ...
        if (response.status_code() >= 400)
        {
            throw ApiException(response.status_code()
                , U("error calling batchUsingPOST: ") + response.reason_phrase()
        ...
        if(response.headers().has(U("Content-Type")))
        {
            ...
            if( contentType.find(responseHttpContentType) == std::string::npos )
            {
                throw ApiException(500
                    , U("error calling batchUsingPOST: unexpected response type: ") + contentType
                    , std::make_shared<std::stringstream>(response.extract_utf8string(true).get()));
            }
        }
